...
The full bundle is now in `reference/orchestrate-stack/`. The three third-party design skills it ships (`anti-slop`, `impeccable`, `design-taste-frontend`) are vendored verbatim under `skills/` but are **published deps his stack invokes, not his authored work**. The authored parts it added:
